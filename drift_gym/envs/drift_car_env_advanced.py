...
@dataclass
class VehicleState:
    """Integrated vehicle state representation."""
    x: float = 0.0
    y: float = 0.0
    theta: float = 0.0
    velocity: float = 0.0
...
class F110VehicleDynamics:
    """
    Integrated F1/10 vehicle dynamics for drift environment.
    Based on UT AUTOmata specifications.
    """
    
    # Physical parameters
    WHEELBASE = 0.324  # meters
    VEHICLE_WIDTH = 0.48  # meters
    MAX_SPEED = 4.219  # m/s
    MAX_TURN_RATE = 0.25  # radians
    ACCEL_LIMIT = 6.0  # m/s^2
    
    # Tire friction parameters
    MU_STATIC = 0.9
    MU_KINETIC = 0.7
    SLIP_THRESHOLD = 3.0  # rad/s
    ...
    def step(self, velocity_cmd: float, angular_velocity_cmd: float) -> VehicleState:
        """
        Step vehicle dynamics forward.
        
        Args:
            velocity_cmd: Commanded linear velocity (m/s)
            angular_velocity_cmd: Commanded angular velocity (rad/s)
        """
        # Apply acceleration limits
        max_accel = self.ACCEL_LIMIT * self.dt
        target_velocity = np.clip(velocity_cmd, -self.MAX_SPEED, self.MAX_SPEED)
        actual_velocity = np.clip(
            target_velocity,
        ...
        # Compute slip factor for drifting
        slip_factor = self._compute_slip_factor(angular_velocity_cmd)
        
        # Apply slip dynamics - during slip, angular velocity is amplified
        actual_angular_velocity = angular_velocity_cmd * (1.0 + slip_factor * 0.3)
    ...
    def _compute_slip_factor(self, angular_velocity: float) -> float:
        """Compute tire slip factor for drifting."""
...
class AdvancedDriftCarEnv(gym.Env):
    ...
    def __init__(
        ...
        # Initialize integrated vehicle dynamics
        self.vehicle = F110VehicleDynamics(dt=0.05, enable_slip=True)
    ...
    def reset(
        ...
        # Reset vehicle
        self.vehicle.reset(x=0.0, y=0.0, theta=0.0)
        
        # Setup scenario obstacles
        self.obstacles = []
        if self.scenario == "loose":
            # Loose drift test: 2.13 meters apart
            self.obstacles.append(Obstacle(x=3.0, y=0.0, radius=0.1))
            self.obstacles.append(Obstacle(x=3.0, y=2.13, radius=0.1))
            self.obstacles.append(Obstacle(x=5.0, y=0.0, radius=0.2))
            self.obstacles.append(Obstacle(x=5.0, y=2.13, radius=0.2))
        else:
            # Tight drift test: 0.81 meters apart
            self.obstacles.append(Obstacle(x=3.0, y=0.0, radius=0.1))
            self.obstacles.append(Obstacle(x=3.0, y=0.81, radius=0.1))
            self.obstacles.append(Obstacle(x=4.5, y=0.0, radius=0.2))
            self.obstacles.append(Obstacle(x=4.5, y=0.81, radius=0.2))
        ...
        if self.use_3d_dynamics:
            ...
            self.vehicle_state_3d = Vehicle3DState(
                x=state.x, y=state.y, z=0.05,
                roll=0.0, pitch=0.0, yaw=state.theta,
                vx=state.velocity, vy=0.0, vz=0.0,
                wx=0.0, wy=0.0, wz=state.angular_velocity
            )
        ...
        if self.use_ekf:
            ...
            initial_state = np.array([
                ...
                state.velocity, 0.0, state.angular_velocity
            ])
    ...
    def step(
        ...
        self.vehicle.step(velocity_cmd, angular_velocity_cmd)
    ...
    def _update_ekf(self):
        ...
        # IMU update
        if self.imu_sensor:
            true_gyro = np.array([0.0, 0.0, state.angular_velocity])
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        if self.use_ekf:
            ...
            state_2d = self.vehicle.get_state()
            x_est = state_2d.x
            y_est = state_2d.y
    ...
    def _check_collision(self) -> bool:
        ...
        state = self.vehicle.get_state()
        vehicle_radius = F110VehicleDynamics.VEHICLE_WIDTH / 2
